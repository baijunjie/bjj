...
export class I18nMergePluginCore {
  ...
  /**
   * Check if file is an i18n source file (must be within srcDirs)
   */
  isI18nFile (id: string): boolean {
    const { languages, srcDirs } = this.options
    const isInSrcDir = srcDirs.some(dir => id.startsWith(dir))
    if (!isInSrcDir) return false
