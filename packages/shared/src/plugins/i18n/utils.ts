...
/**
 * Parse --packages arguments from process.argv.
 *
 * Supports:
 ...
 *
 * // Special handling for [locale] and i18n segments
 * pathToNamespace('/project/app/pages/[locale]/dashboard/en.json', '/project/app')
 * // Returns: 'pages.dashboard' (removes [locale])
 *
 * // i18n folder is also removed from paths
 * pathToNamespace('/project/app/pages/dashboard/i18n/en.json', '/project/app')
 * // Returns: 'pages.dashboard' (removes i18n)
 *
...
export function pathToNamespace (
  ...
  // Remove [locale] and i18n from the path while preserving other directory names exactly
  const cleanedDir = dir
    .replace(/\[locale\]/g, '') // Remove [locale] segments for i18n routing
    .replace(/(?:^|\/)i18n(?:\/|$)/g, '/') // Remove i18n folder name from path
